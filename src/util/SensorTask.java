...
//       Location date = (Location) locations.get(i);
...
//         Date installationDate = DateManager.removeTime(sensore.getInstallation_date());
         ...
         OffsetDateTime installationDate = sensore.getInstallation_date().toInstant().atOffset(ZoneOffset.UTC).truncatedTo(ChronoUnit.DAYS);
         OffsetDateTime dataPrimaMisura = getFirstMeasure(location).toInstant().atOffset(ZoneOffset.UTC);
         OffsetDateTime endTime = dataPrimaMisura;
         OffsetDateTime startTime = dataPrimaMisura.minusDays(Constant.PERIOD_PAST_REQUEST);

//         Date startTime = DateManager.addDays(dataPrimaMisura, -Constant.PERIOD_PAST_REQUEST);
//         Date endTime = dataPrimaMisura;
         SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd-HH:mm:ss");
         while (!startTime.isBefore(installationDate)) {
            System.out.println("startdate" + startTime.truncatedTo(ChronoUnit.DAYS));
            ...
            String st = startTime.format(DateTimeFormatter.ofPattern("yyyy-MM-dd-HH:mm:ss"));
            String et = endTime.format(DateTimeFormatter.ofPattern("yyyy-MM-dd-HH:mm:ss"));
            //System.out.println("loc_id = " + sensorLocation);
            ...
            System.out.println("requestPCA_AVG = " + requestPCA_AVG);
            List<Object> misure = new ArrayList<>();
//            RequestSensorData req1 = new RequestSensorData();
//            RequestSensorData req2 = new RequestSensorData();
//            RequestSensorData req3 = new RequestSensorData();
...
//            List<Measurement> m1 = req1.getData(requestPCA, date, mappaParametri.get(0));
//            List<Measurement> m2 = req2.getData(requestPCB, date, mappaParametri.get(0));
//            List<Measurement> m3 = req3.getData(requestSCA, date, mappaParametri.get(1));
//            List<Measurement> m4 = req4.getData(requestSCB, date, mappaParametri.get(1));
...
//            misure.addAll(m3);
//            misure.addAll(m4);
            misure.addAll(doPastAvgPeriod(m1AVG, m3AVG, DateManager.truncate(startTime, Constant.PERIOD_AVG), DateManager.truncate(endTime, Constant.PERIOD_AVG)));
            ...
            endTime = startTime;
            startTime = startTime.minusDays(Constant.PERIOD_PAST_REQUEST);
            System.out.println("");
            System.out.println("");
...
//        System.out.println("this = ");
   ...
   private Date getFirstMeasure(Location l) {
      ...
      List<Object> date = ManagerDB.getObjectList(Query.GET_FIRST_MEASURE, c);

      Date data = new Date();
      if (!date.isEmpty()) {
         data = (Date) date.get(0);
      }
...
//      Date dataFinale = DateManager.removeTime(data);
   ...
   private List<Measurement> getMeas(List<Measurement> l, OffsetDateTime d) {
      List<Measurement> tmp = new ArrayList<>();
      for (Measurement m : l) {
         if (parametri_elaborati.get(m.getParameter().getCode()) != null) {
            if (m.getTimestamp().toInstant().atOffset(ZoneOffset.UTC).equals(d)) {
               tmp.add(m);
            }
         }
      }
   ...
   private List<Measurementavg> doPastAvgPeriod(List<Measurement> m1, List<Measurement> m3, OffsetDateTime start, OffsetDateTime end) {
      OffsetDateTime time = start;
      List<Measurement> tmp1 = new ArrayList<>(), tmp3 = new ArrayList<>();
      List<Measurementavg> avgM = new ArrayList<>();
      while (time.isBefore(end)) {
         tmp1.addAll(getMeas(m1, time));
         tmp3.addAll(getMeas(m3, time));
         avgM.addAll(doAvgChannels(tmp1, tmp3));
         time = time.plusMinutes(Constant.PERIOD_AVG);
         tmp1.clear();
         tmp3.clear();
      }
      return avgM;
...
//                System.out.println("m3 = " + measurement.getTimestamp());
      ...
      for (String entry : parametri_elaborati.keySet()) {
         ...
         } else if (map1.get(entry) == null && map3.get(entry) != null) {
            System.out.println("map3 " + map3.get(entry).getTimestamp());
            mis.add(addMisuraElaborata(map3.get(entry), map3.get(entry).getValore(), parametri_elaborati.get(entry)));
         } else if (map1.get(entry) != null && map3.get(entry) == null) {
            System.out.println("map1 " + map1.get(entry).getTimestamp());
            mis.add(addMisuraElaborata(map1.get(entry), map1.get(entry).getValore(), parametri_elaborati.get(entry)));
         }
      }
